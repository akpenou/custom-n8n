...
export default class GenericLayout {
	...
	public appendContentRequests(requests: SlidesV1.Schema$Request[]): SlidesV1.Schema$Request[] {
		...
		if (this.slide.bodies) {
			...
			const bodyCount = Math.min(bodyElements?.length ?? 0, this.slide.bodies.length);
	...
	protected appendCreateImageRequests(
		...
		for (const item of computedLayout.items) {
			...
			const translateX = baseTranslateX + (item.x + itemPadding + itemOffsetX) * scaleRatio;
			const translateY = baseTranslateY + (item.y + itemPadding + itemOffsetY) * scaleRatio;
	...
	protected getBodyBoundingBox(placeholder: SlidesV1.Schema$PageElement | undefined): BoundingBox {
		if (placeholder) {
	...
	protected computeShallowFieldMask<T>(object: T): string {
		const fields = [];
		for (const field of Object.keys(object as any)) {
